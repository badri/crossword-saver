        ...
        if 'grid_id' in request.GET:
            ...
            for i,j in enumerate(grid):
                if i+1 in number_info.keys():
                    crossword.append({'grid':j, 'code': number_info[i+1]['code'], 'clue': number_info[i+1]['clue']})
                else:
                    crossword.append({'grid':j, 'code': '', 'clue': ''})
                    
            print crossword
            return render_to_response('crossword.html', {'crossword':crossword})
        else:
