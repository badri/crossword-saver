...
*/
(function($){
  $.fn.header = function(metaData) {
      var header = ['<div id="setter"><h2>Crossword #' + metaData.id + ' - ' + metaData.author + ' </h2></div>']
      header.push('<div id="date"><h2>' + metaData.date + '</h2></div>')

      this.append(header.join(''))
  }
  ...
  $.fn.crossword = function(entryData) {
      ...
      */
      ...
      this.after('<div id="puzzle-clues"><div id="current-clue"></div><h2 >Across</h2><ul id="across"></ul><h2>Down</h2><ul id="down"></ul></div>');
      ...
      var puzInit = {
        ...
        init: function() {
          ...
          currOri = puzz.data[0].orientation; // app's init orientation could move to config object
          util.updateCurrentClue();
          ...
          for (var i=1; i <= rows; ++i) {
            tbl.push("<tr>");
              for (var x=1; x <= cols; ++x) {
                tbl.push('<td class="table_entry" data-coords="' + x + ',' + i + '"></td>');    
              };
            tbl.push("</tr>");
          };
        ...
        buildEntries: function() {
          ...
          for (var x=1, p = entryCount; x <= p; ++x) {
            ...
            for (var i=0; i < entries[x-1].length; ++i) {
              ...
              // TODO: Something might be fishy here. Check again
              if(x > 1 ){
              ...
              if($(light).empty()){
                $(light)
                  ...
                  .append('<input class="inputbox" style="text-transform:uppercase;" maxlength="1" val="" type="text" tabindex="-1" />');
              }
            };
          ...
          for (var i=1, p = entryCount; i <= p; ++i) {
            ...
            if(!$('.entry-' + i +':eq(0) span').length){
              ...
              $groupedLights.eq(0)
                .append('<span class="markup">' + puzz.data[i-1].position + '</span>');
            }
          } 
        ...
        */
        checkAnswer: function(e) {
          ...
          // TODO: This is a hack to disable answer checking
      ...
      var util = {
        ...
        updateCurrentClue: function() {
            //$('#current-clue').empty().append(puzz.data[activePosition].position + '. ' + puzz.data[activePosition].clue); 
            $('#current-clue').empty().append(puzz.data[activePosition].clue); 
          },
